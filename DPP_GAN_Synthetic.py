...
def evaluate_samples(generated_samples, data, model_num, is_ring_distribution=True):
    ...
    plt.axis('off')
    plt.savefig(write_dir + 'Plots/DPPGAN_{}_Final.png'.format(model_num))
...
def run_gan(model_num):
    ...
    with tf.Graph().as_default():
        ...
        for i in tqdm(xrange(params['max_iter'])):
            [d_loss, g_loss], _, _, f_lmda, r_lmda, div_loss, eig_vals_loss_curr, eig_vecs_loss_curr = sess.run(
                [[disc_loss, gen_loss], g_train_op, d_train_op, fake_lambda, real_lambda, diversity_loss, eig_vals_loss, eig_vecs_loss])
            ...
            if i % params['viz_every'] == 0:
                ...
                diversity_loss_arr.append(div_loss)
                gen_loss_arr.append(g_loss - div_loss)
                disc_loss_arr.append(d_loss)
                h5f = h5py.File(write_dir + 'Plots/DPPGAN_{}.h5'.format(model_num), 'w')
        ...
        xx = np.vstack([sess.run(samples) for _ in range(5)])
